<?php
	...
	class Group extends ResultElement implements ElementParent
	{
		 ...
		 * @param	callable|null	$children	A callable for adding items directly to this group
		 */
		public function __construct(string $name, ?callable $children = null)
		{
		...
		{
			$json = parent::getProps() + ["children" => []];
			
			foreach($this->children as $child)
			{
				$childJson = $child->getProps();
		...
		public function getDefaultGeneratorLabel(): array
		{
			...
			foreach($this->children as $child)
			{
				$labels[$child->name] = $child->getDefaultGeneratorLabel();
			}
