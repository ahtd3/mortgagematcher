<?php
	...
	class GeneratorElement extends ResultElement
	{
		...
		/**
		 * Generates a temporary ID for a generator. We'll use this to associate individual generators with their labels, so it needs to be deterministic
		 * @param	Generator	$generator	The generator to generate the temporary ID for
		 * @return	string					The temporary ID for that generator
		 */
		private static function generateTemporaryIdFor(Generator $generator): string
		{
			return md5(spl_object_hash($generator));
		}
		...
		public function getProps(): array
		{
			...
			if(!is_array($value))
			{
				...
				{
					$json["elements"][] = $element->getProps();
				}
			}
			...
			else
			{
				...
				$newGeneratorJson = $newGenerator->getProps();
				...
				$json["newGeneratorLabel"] = json_encode($newGenerator->getDefaultGeneratorLabel());
				...
				foreach($value as $index => $generator)
				{
					...
					$generatorJson = $element->getProps();
					$generatorJson["id"] = $generator->id;
					$generatorJson["temporaryId"] = self::generateTemporaryIdFor($generator);
					...
					$json["childGenerators"][] = $generatorJson;
				}
			}
		...
		public function getDefaultGeneratorLabel(): ?array
		{
			$value = $this->getValue();
			$labels = [];
			...
			if(is_array($value))
			{
				...
				{
					assert($item instanceof Generator);
					
					foreach($item->getElements() as $element)
					{
						$temporaryId = self::generateTemporaryIdFor($item);
						$labels[$temporaryId][$element->name] = $element->getDefaultGeneratorLabel();
					}
				}
			}
			else
			{
				assert($value instanceof Generator);
				
				foreach($value->getElements() as $element)
				{
					$labels[$element->name] = $element->getDefaultGeneratorLabel();
				}
			}
