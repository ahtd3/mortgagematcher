<?php
	...
	abstract class Entity implements EntityInterface, JsonSerializable, Searchable
	{
		...
		/**
		 * Handles generating data for serialisation. Mostly used for dumping objects into sessions.
		 * @return	array	The data we're serialising. Note that unlike json_encode(), PHP serialisation allows recursion, so we don't need to worry about cyclic dependencies
		 */
		public function __serialize()
		{
			static $entityReferences = []; // Special PHP magic that creates or retrieves a static variable that is only available to this function
			$shouldResetEntityReferences = (count($entityReferences) === 0);
			$reference = spl_object_hash($this);
			$entityReferences[$reference] = $this;
			
			$data = ["__ref__" => $reference];
			
			$convertLink = function(Entity $entity) use($entityReferences)
			{
				$reference = spl_object_hash($entity);
				
				// If the entity is already in the database, we just need to store its ID, rather than the entire entity
				if($entity->id !== null || $entity->isNull())
				{
					return $entity->id;
				}
				// Avoid circular dependencies, we've already got this object in the array, so we can just pull it back out
				else if(isset($entityReferences[$reference]))
				{
			...
			{
				// We don't want to store more data than necessary, so we'll convert anything that's already in the database into its ID
				if($valueWrapper instanceof LinkWrapper)
				{
					$value = $valueWrapper->getForOutput();
					
					if(is_array($value)) $data[$name] = array_map($convertLink, $value);
					else $data[$name] = $convertLink($value);
				}
				else if($valueWrapper->property->getDatabaseName() === null)
				{
					// Ignore dynamic properties
				}
				else
				{
					$data[$name] = $valueWrapper->getForDatabase();
				}
			}
			
			// If this was the initial entry point, we want to empty this array, so that this doesn't interfere with other serialisation attempts, and doesn't cause memory leaks
			if($shouldResetEntityReferences) $entityReferences = [];
			
			return $data;
		}
		
		/**
		 * Handles converting serialised data back into an actual object. Mostly used for restoring objects from sessions. This could cause issues if the session object matches one in the database, so it would be best to only serialise objects that haven't been saved to the database.
		 * @param	array	$data	The original serialised data (serialised by __serialize())
		 */
		public function __unserialize(array $data)
		{
			static $entityReferences = []; // Magic again, see __serialize()
			$shouldResetEntityReferences = (count($entityReferences) === 0);
			$entityReferences[$data["__ref__"]] = $this;
			
			// This will set up a fresh copy of all the attributes
			$this->__construct();
			
			$convertLink = function(LinkWrapper $wrapper, mixed $value) use($entityReferences): Entity
			{
				/** @var class-string<Entity> $type */
				$type = $wrapper->property->getType();
				
				// The item is already stored in the database
				if(is_int($value))
				{
					return $type::load($value);
				}
				// The item has already been deserialised, and just needs to be retrieved from our handy array
				else if(is_array($value) && $value[0] === "__ref__" && isset($entityReferences[$value[1]]))
				{
					return $entityReferences[$value[1]];
				}
			...
			foreach($this->valueWrappers as $name => $valueWrapper)
			{
				...
				if(!$valueWrapper instanceof LinkWrapper) $valueWrapper->setFromDatabase($value);
				else if(is_array($value) && ($value[0] ?? null) !== "__ref__") $valueWrapper->setFromInput(array_map(fn(array $item) => $convertLink($valueWrapper, $item), $value));
				else $valueWrapper->setFromInput($convertLink($valueWrapper, $value));
			}
			
			// Empty this for the same reason as in __serialize()
			if($shouldResetEntityReferences) $entityReferences = [];
		}
