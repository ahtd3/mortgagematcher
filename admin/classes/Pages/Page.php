<?php
	...
	class Page extends Generator implements AdminNavItemGenerator, BannerSource, NavItem, PageItem, SearchResultGenerator, ShortCodeSupport, SitemapItem
	{
		...
		public static function getHomepagePath()
		{
			...
			{
				$page = PageType::getPageOfType(PageType::FRONT_PAGE);
			}
		...
		protected function elements()
		{
			...
			$this->addElement(new Text("redirect", "Redirect URL", $this->redirect), "Setup")->setConditional("return actsAs === 'externalLink'")->setResultHandler(function(string $url)
			{
				// We're doing this inside the result handler instead of a validation because the "isExternalRedirect" value needs to be set before we can do this check
				if($this->isExternalRedirect && trim($url) === "") throw new Exception("You must enter a URL to redirect to");
				else $this->redirect = $url;
			});
			
			$this->addElement(new Select("original", "Link to / duplicate of", $this->getPossibleLinks()), "Setup")->setConditional("return actsAs === 'internalLink' || actsAs === 'duplicate'")->setResultHandler(function($page)
			{
				// We're doing this inside the result handler instead of a validation because the "isInternalRedirect" and "isDuplicate" values needs to be set before we can do this check
				if($this->isInternalRedirect && !$page) throw new Exception("You must choose a page to redirect to");
				if($this->isDuplicate && !$page) throw new Exception("You must choose a page to duplicate");
				else $this->setFromElement("original", $page);
			});
