	...
	class EditController extends AdminController
	{
		...
		protected static function getControllerFromPattern(UrlController $parent = null, array $matches = [], $pattern = "")
		{
			...
			else if(isset($matches['element']))
			{
				...
				// Tabs and sections may have the same name as a descendant element, but we're typically looking to reload that descendant
				while($element instanceof Group)
				{
					$element = $element->findElement($matches["element"]);
				}
				
				return new JsonController(['props' => $element->getProps()]);
			}
			...
			else
			{
				...
				foreach($generator->getElements() as $element)
				{
					...
					$json["generatorLabels"][$element->name] = $element->getDefaultGeneratorLabel();
				}
